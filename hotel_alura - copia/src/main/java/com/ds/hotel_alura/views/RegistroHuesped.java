...
import static com.ds.hotel_alura.views.ReservasView.txtFechaSalida;
...
public class RegistroHuesped extends JFrame {
    ...
    int xMouse, yMouse;
    private ReservaDTO reservasDTO;
    ...
    private boolean isEdadRight = false;

    public void setReservasDTO(ReservaDTO dto) {
        reservasDTO = dto;
        txtNreserva.setEditable(false);
        txtNreserva.setText("00" + reservasDTO.id());
    }
    ...
    public RegistroHuesped() {
        ...
        txtFechaN.addPropertyChangeListener(new PropertyChangeListener() {
            public void propertyChange(PropertyChangeEvent evt) {
                ...
                if (txtFechaN.getCalendar() != null) {
                    var year = txtFechaN.getCalendar().get(Calendar.YEAR);
                    IDateEditor dateEditor = txtFechaN.getDateEditor();
                    if (dateEditor instanceof JTextFieldDateEditor) {
                        if (LocalDateTime.now().getYear()-year<18) {
            ...
            public void mouseClicked(MouseEvent e) {
                if(isEdadRight){
                    ...
                    huespedDTO = new HuespedDTO(
                            0, txtNombre.getText(), txtApellido.getText(),
                            sdf.format(txtFechaN.getDate()),
                            txtNacionalidad.getSelectedItem().toString(),
                            txtTelefono.getText(), reservasDTO.id()
                    );
                    call.listener(huespedDTO, (byte) 0);
                    dispose();
                }else{
                    JOptionPane.showMessageDialog(null, 
                        "La acción no se puede completar debido a que usted"
                        + "no es mayor de edad, por favor ponga su edad correcta");
                }
            }
        });
